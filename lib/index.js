...
class Client {
   ...
   * @param queue
   */
  constructor(addr, queue, opts){

    this.$client =  amqp.client(addr, queue);
    this.$syncClient = deasync(this.$client);

    this.$definition = this.$syncClient(request(rpc.DEFINITIONMETHOD), opts);
 ...
 * @returns {*}
 */
module.exports = function(addr, queue, opts = defaultOpts){

  const hash = generateHash(addr, queue);

  clients[hash] = clients[hash] || new Client(addr, queue, opts);
